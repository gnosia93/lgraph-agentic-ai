...
promptfoo는 LLM을 성능 평가를 위한 단위 테스트 프레임워크입니다. 소프트웨어 개발에서 코드를 변경할 때마다 단위 테스트를 돌려 로직이 제대로 동작하는지 확인하듯이, promptfoo는 프롬프트나 모델을 변경할 때마다 평가를 실행하여 응답 품질이 유지되는지 확인할 수 있게 해줍니다. 프롬프트, 비교할 모델 목록, 테스트 케이스를 하나의 YAML 파일에 선언적으로 정의하면, 여러 모델 × 여러 테스트 케이스 × 여러 평가 기준을 한 번에 실행하고 그 결과를 비교할 수 있습니다.
