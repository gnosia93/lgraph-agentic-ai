...
물론 LLM이 LLM을 채점하는 방식에 한계가 없는 것은 아닙니다. 채점 모델의 판단이 늘 완벽하지는 않기에, 평가 기준을 어떻게 설계하고 threshold를 어디에 둘지는 여전히 사람의 몫으로 남습니다. 그럼에도 promptfoo는 여러 모델과 여러 테스트, 여러 기준의 조합을 한 번의 실행으로 자동 비교할 수 있는 틀을 제공함으로써, LLM을 다루는 일을 추측이 아닌 측정의 영역으로 끌어올려 줍니다. 작은 설정 파일 하나부터 직접 시작해 보면, 프롬프트와 모델을 다루는 방식이 한층 단단해지는 것을 느낄 수 있을 것입니다.
