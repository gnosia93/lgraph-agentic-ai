...
EFA를 실제로 쓰려면, 아래 설정이 필요하다. 
* EFA 스택이 포함된 이미지 사용 — 기본 vLLM 이미지에 EFA installer + aws-ofi-nccl을 추가해 커스텀 빌드하거나, EFA가 포함된 베이스(예: AWS Deep Learning Containers) 위에 vLLM을 올리는 방식
* 클러스터에 EFA device plugin 설치 — vpc.amazonaws.com/efa 리소스를 노출시켜야 함
* 매니페스트에서 EFA 리소스 요청 — 주석을 풀어 vpc.amazonaws.com/efa를 limits에 명시
* (옵션) NCCL 환경변수 설정 — FI_PROVIDER=efa 등으로 EFA 경로를 명시
