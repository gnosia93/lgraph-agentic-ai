## 오픈소스 100B Omni-VLM 추론 - 지식 증류 데이터 생성 ##
...
### 2. GPU 구성 — 추론 메모리 계산 및 설정 ###

추론은 학습과 메모리 프로파일이 다릅니다. 옵티마이저/그래디언트/활성화 저장이 없는 대신,
**가중치 + KV 캐시 + 멀티모달 인코더 오버헤드**가 VRAM을 지배합니다. "100B 교사를 몇 장에 올리나?"는
이 셋을 계산하면 답이 나옵니다.

### 2.1 추론 메모리의 3대 구성요소
| 구성요소 | 크기 | 비고 |
|----------|------|------|
| 모델 가중치 | bf16 = 2P, fp8 = 1P | 100B면 bf16 ≈ 200GB, fp8 ≈ 100GB |
| **KV 캐시** | `2 · L · n_kv · d_head · seq · batch · dtype` | 동시 처리량(throughput)을 좌우 |
| 활성화 + 멀티모달 인코더 | 상대적으로 작음(비전/오디오 인코더 forward) | 배치·해상도에 비례 |

> 학습의 "파라미터당 16바이트"와 달리, 추론은 **가중치(1~2P) + KV 캐시**가 핵심입니다.

#### 2.2 가중치 메모리와 TP 분할 (100B 기준) ####
- bf16: 2 × 100e9 ≈ **200 GB** → 80GB GPU 1장 불가.
- **TP=8 분할**: 200 / 8 = **GPU당 약 25 GB**. 80GB 카드면 나머지 ~55GB를 KV 캐시/활성화로 쓸 수 있음.
- **fp8 양자화(H100급)**: 100 GB → TP=8이면 **GPU당 약 12.5 GB**. 가중치 여유분이 그대로 KV 캐시
  용량 = 동시 배치 수 증가로 이어져 스루풋이 올라갑니다.

### 2.3 KV 캐시 = 스루풋의 병목
KV 캐시는 시퀀스 길이·동시 요청 수에 비례해 선형 증가합니다. 멀티모달은 이미지/비디오/오디오 토큰이
컨텍스트를 크게 늘리므로 KV 캐시 압박이 텍스트보다 훨씬 큽니다.

```
KV_per_token ≈ 2(K,V) · L · n_kv_heads · d_head · dtype_bytes
전체 KV ≈ KV_per_token · Σ(시퀀스 길이) · 동시 요청 수
```

- vLLM의 `gpu_memory_utilization`(예: 0.90)은 **가중치를 뺀 나머지 중 KV 캐시로 쓸 비율**을 정합니다.
- `max_model_len`을 키우면 요청당 KV 상한이 커져 **동시 처리 배치 수가 줄어드는** 트레이드오프.
- GQA(Grouped-Query Attention) 모델은 `n_kv_heads`가 작아 KV 캐시가 훨씬 가벼움(옴니 모델 선택 시 이점).

#### 2.4 카드/토폴로지 선택 가이드 ####
| 목표 | 구성 | 근거 |
|------|------|------|
| 100B bf16 추론 | A100/H100 **80GB × 8 (TP=8)** | 가중치 25GB/장 + KV 여유 |
| 스루풋 극대화 | H100 80GB × 8 + **fp8** | 가중치 반감 → KV 캐시 2배 → 동시 배치↑ |
| 예산 절감/자체 클러스터 | A100 80GB × 8 + AWQ/INT8 | A100은 fp8 텐서코어 없음 → INT8 계열 |
| 40GB 카드만 가능 | TP=8로도 가중치 25GB는 들어가나 KV 여유 부족 | 배치 작게, `max_model_len`↓ 필요 |

- **노드 내부는 TP=8**(NVLink), 노드를 넘기면 통신비용 때문에 **다중 인스턴스(데이터 병렬 복제)**로
  스루풋을 늘리는 편이 배치 추론에 유리합니다.
- fp8은 **Hopper(H100/H200)** 네이티브. A100은 fp8 텐서코어가 없어 AWQ/GPTQ/INT8을 사용.

#### 2.5 설정으로 메모리·스루풋을 조절하는 레버 ####
| 목표 | 조절할 설정 |
|------|-------------|
| 가중치 절감(→KV 여유) | `dtype="fp8"` 또는 AWQ/INT8 양자화 |
| KV 캐시 용량↑ | `gpu_memory_utilization`↑ (0.85→0.92), 불필요 프로세스 정리 |
| 요청당 상한 관리 | `max_model_len` 조절(멀티모달은 32k↑ 권장, 단 배치와 트레이드오프) |
| 모달 개수 상한 | `limit_mm_per_prompt`로 프롬프트당 이미지/오디오/비디오 상한 |
| OOM 회피(즉효) | `max_num_seqs`↓, `max_model_len`↓, 배치 청크 축소 |
| 멀티노드 스루풋 | TP는 노드 내 8로 고정, 노드 간은 인스턴스 복제로 확장 |

> 실측 팁: 엔진 로딩 로그의 "KV cache blocks" 수와 `gpu_memory_utilization`을 함께 보며 조정하세요.
> 블록이 너무 적으면 동시 배치가 막혀 스루풋이 떨어지고, `max_model_len`을 낮추면 회복됩니다.
