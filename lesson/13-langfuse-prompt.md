...
대상: 애플리케이션 개발자, DevOps 엔지니어
선행: 1편(설치와 첫 트레이스), 2편(멀티스텝 트레이싱)
...
이 편에서는 프롬프트를 Langfuse에서 버전 단위로 관리하고 트레이스와 연결하는 방법을 다뤘다. 다음 편에서는 출력 품질에 점수를 부여하는 평가(Evals)를 다룬다. 사람이 직접 점수를 매기는 방식과 LLM이 자동으로 평가하는 방식, 그리고 그 점수를 시간과 버전에 따라 추적하는 방법을 살펴본다.
